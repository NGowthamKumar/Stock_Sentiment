...
# ================================ MODEL HEALTH =======================
with tab4:
    st.subheader("Model Health & Notes")

    st.write("""
    - Predictions are generated by the best of Ridge / RandomForest using TimeSeriesSplit CV.
    - Direction accuracy improves with more history (target 55–60%+). Retrain weekly.
    - Confidence proxy = |predicted return| / MAE (tune MAE after training).
    - Use filters in the Predictions tab to avoid weak/noisy signals.
    """)

    # (Optional) If you log CV metrics to a CSV, you can show them here.
    # metrics = load_csv("data/modeling/cv_metrics.csv")
    # if not metrics.empty:
    #     st.dataframe(metrics.sort_values("run_ts", ascending=False).head(5), use_container_width=True)
...
with tab5:
    ...
    **What this app does, end-to-end:**  
    ...
    with colA:
        ...
        - **GitHub Actions / Task Scheduler**: simple, reliable automation for hourly refresh and weekly retraining.
        """)

        # ---------- 3) Automation & retraining ----------
        st.markdown("### Automation & Retraining")
        st.markdown("""
        - **Ingestion & scoring**: run **hourly** during India market hours (IST 09:00–16:00) → updates SmartScores & signals.  
        - **Backfill**: **nightly** (optional) to keep rolling history consistent.  
        - **Model retraining**: **weekly** (Fri 18:00 IST) or after ≥5 new trading days.  
        ...
        st.markdown("""
        Deployed on: **<add your host: Streamlit Cloud / Hugging Face Spaces>**  
    ...
    with colB:
        ...
        st.code(
            ...
            "src/sentiment_vader.py      # FinBERT+VADER ensemble\n"
            "src/event_classifier.py     # rule-based event tags\n"
            "src/aggregate_sentiment.py  # SmartScore v2 (0–100)\n"
            "src/backfill_history.py     # rebuild daily snapshots (past N days)\n"
            "src/build_dataset.py        # join features with prices, create labels\n"
            "src/train_regression.py     # Ridge/RF + CV, save best model\n"
        ...
        st.caption("Tip: Automate fetch→sentiment→aggregate→predict hourly in market hours; retrain weekly.")
